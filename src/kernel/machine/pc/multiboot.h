#ifndef KERNEL_MACHINE_PC_MULTIBOOT_H_
#define KERNEL_MACHINE_PC_MULTIBOOT_H_
...
struct __attribute__((__packed__)) MULTIBOOT_BOOTINFO {
    uint32_t flags;
...
struct __attribute__((__packed__)) MULTIBOOT_BOOTINFO_MMAP {
  uint32_t size;
  uint64_t addr;
  uint64_t len;
  uint32_t type;
...
struct __attribute__((__packed__)) MULTIBOOT_BOOTINFO_MODULES {
  uint32_t start;
  uint32_t end;
  uint32_t name;
  uint32_t reserved;
...
#endif  // KERNEL_MACHINE_PC_MULTIBOOT_H_
